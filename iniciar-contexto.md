...
- Ao mover uma seção, deixe uma nota curta no lugar de onde ela saiu (ex.: "Seção X movida para `context.md`") e corrija toda referência cruzada que apontava para ela (`ver seção X` vira `ver seção X em context.md`). Não deixe ponteiros órfãos.
